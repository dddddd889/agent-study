...
> 📈 **本仓库按步骤演进。** 当前代码已实现到 **第 7 步：中断回复**。步骤文档：
>
...
> - 第 7 步 · 中断回复（Ctrl+C 打断本轮）→ [docs/07-interrupt.md](docs/07-interrupt.md)
>
...
这个内核刻意保持简单，方便往上长。推荐的演进顺序：

1. **流式输出**：把 `complete` 改成返回流，边生成边打印，体验更好。
2. ✅ **工具调用（function calling）**：让模型能调用你定义的函数（查天气、读文件……），循环里多一个"如果模型要调工具 -> 执行 -> 把结果喂回去"的分支。这是从"聊天机器人"变成"agent"的关键一步。**已完成，见 [docs/02-tool-calling-loop.md](docs/02-tool-calling-loop.md)。**
3. **历史裁剪 / 摘要**：对话变长后控制 token，比如保留最近 N 轮 + 早期摘要。
4. **多 LLM 后端**：再写一个实现 `LLM` 接口的类（如 OpenAI），Agent 代码一行不用改。
5. **持久化与会话管理**：把 history 存盘/存库，支持多会话。
6. **规划与子任务**：让 agent 把复杂目标拆成步骤逐个执行。
