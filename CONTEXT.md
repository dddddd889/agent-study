...
**流水（log）**：
一条 append-only、一行一条、只追不改的落盘序列，是所承载历史的**唯一真相**。会话流水、子档案都是它的具体形态；与之相对的是可随时重写、丢了能自愈的**派生缓存**（如记忆游标 sidecar）——派生缓存不是流水。
_Avoid_：日志、记录
...
**冻结区重建（restoreFrozen）**：
[[摘要缓存]] 的**读取侧**对称操作：续聊启动时读回 sidecar、校验游标对主流水合法（结构自洽 + 末游标落在轮边界），命中则以「冻结块 + 游标之后逐字」重建工作历史，不命中退回全量。写入侧是 [[增量冻结]] 与 [[合并]]。缺失属常态（静默），损坏或校验不过属坏缓存（告警但仍照常恢复）。
_Avoid_：续聊重建、重建工作历史

### 长期记忆

**长期记忆（long-term memory）**：
跨会话沉淀的【事实 / 偏好 / 决定】，存成单个 Markdown 文件（默认 `.memory.md`），会话开始时注入系统提示。与 [[记忆游标]] 无关——两者都叫「记忆」，但长期记忆跨会话攒事实、记忆游标是单会话内的压缩位置标记。
_Avoid_：记忆（单说时与记忆游标混）
